...
class ParkingLot:
    ...
    def park_vehicle(self, vehicle_number, vehicle_type):
        ...
        try:
            if vehicle_number in self.parking_data:
                ...
                logging.info(f"{vehicle_type.capitalize()} {vehicle_number} parked successfully.")
    ...
    def calculate_charges(self, vehicle_number):
        ...
        try:
            ...
            logging.info(f"{vehicle_type.capitalize()} {vehicle_number} parked for {hours_parked} hour(s). Total charges: ₹{charges}")
...
def handle_vehicle(parking_lot, vehicle_number, vehicle_type, action):
    ...
    if action == "park":
        ...
        logging.warning(f"Invalid action for vehicle {vehicle_number}.")
